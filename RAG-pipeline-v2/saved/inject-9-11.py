...
def load_and_split_documents(pdf_directory):
    """Loads PDFs, splits them, and adds an ingestion timestamp to metadata."""
    ...
    print(f"Added 'ingested_at' timestamp to all chunks: {ingestion_time}")
    # -----------------------------------------
...
if __name__ == "__main__":
    ...
    try:
        # Note: RESET_COLLECTION is now controlled by environment variable
        # To reset, run: RESET_COLLECTION=1 python inject.py
        #if os.getenv("RESET_COLLECTION", "0") == "1":
        if RESET_COLLECTION:
